...
class Multitasking extends CI_Controller implements multitMethod  {
    ...
    public function index(){
       ...
       * */
        ...
        $this->view_library->multitask('multitask', $data);
    ...
    public function add(){
        if(isset($_POST) && sizeof($_POST) > 0){
            ...
            if(!empty($_FILES)){
                $filename = $_FILES['document']['name']; // Получаем изображение
            ...
            if(!empty($result)){
                // Подключение смс оповещения сотрудников
                require('./assets/sms/transport.php');
                $api = new Transport();
                $params = array(
                ...
                if($_SERVER['SERVER_NAME'] != 'tasks'){
                    for($j = 0; $j < $count; $j++){
                        $phones[$j] = array("".$getTaskForNotification[$j]->phone."");
                        $api->send($params,$phones[$j]);
                    }
                }
            ...
            $this->view_library->multitask('add', $data);
        }
    }
    ...
    public function edit($task_id = null){
        ...
        if(isset($_POST) && sizeof($_POST) > 0){ // Проверяем пришел ли POST
            ...
            if(!empty($_FILES)){
                ...
                $type = $_FILES['document']['type'];
                $size = $_FILES['document']['size'];
                if($size > MAX_SIZE_PICTURE){ // 8 Мб - масимум
                    echo json_encode(array('status' => 'error_size')); // Размер картинки или файла превышает 3 Мегабайт
                    exit;
                } else if($type != 'image/jpeg' && $type == 'image/png' && $type != 'image/jpg' && $type != 'text/plain'){
                    echo json_encode(array('status' => 'error_no_correct')); // Неккоректный тип изображения или документа
                    exit;
                }
                $ext = substr($filename, 1 + strrpos($filename, ".")); // получаем его расширение
            ...
            if($result == true){
                ...
                $api = new Transport();
                $params = array(
                    "text" =>"Добавлена задача!\r\nnew.buket116.ru/m/".$task_id."\r\nВыполнить до ".date("d-m-y H:i", strtotime($new_time)).""
                );
                if(isset($_POST['moder_status']) && $_POST['moder_status'] != 0) {
                    if ($post['moder'] == 2) {
                        $count = count($edit_multitask);
                        for ($j = 0; $j < $count; $j++) {
                            $phones[$j] = array("" . $edit_multitask[$j]->phone . "");
                            //$send = $api->send($params, $phones[$j]);
            ...
            $data['multitask'] = 'multitask';
            $this->view_library->multitask('edit', $data);
        }
    }
    ...
    public function review($task_id = null){
        ...
        if(isset($_POST) && sizeof($_POST) > 0){
            ...
            if(empty($data['review_multitask'])){
                ...
                $this->view_library->multitask('review', $data);
            }
        }
    }
    ...
    public function all_tasks(){
        ...
        $tasks = $this->cache->get('user_'.USER_COOKIE_ID);
        if(empty($tasks)){
            $post['department'] = null;
            $load['load'] = 0;
            $data['selectTasksEmployees']['result'] = $this->multitasking_model->getTasksAllUsers($post, $load);
            $data['count_tasks'] = count($data['selectTasksEmployees']['result']);
            //echo "<pre>";print_r($data['count_tasks']);exit;
        } else {
            $data['selectTasksEmployees'] = $this->cache->get('user_'.USER_COOKIE_ID);
            $data['count_tasks'] = count($data['selectTasksEmployees']['result']);
            //echo "<pre>";print_r($data['selectTasksEmployees']['result']);exit;
            //echo "<pre>";print_r($data['count_tasks']);exit;
        }*/
        $this->view_library->multitask('all_tasks', $data);
    }
    ...
    public function getNewSelectAllTasks(){
        ...
        if(!empty($result)){
            /*$this->load->driver('cache', array('adapter' => 'apc', 'backup' => 'file'));
            if ( ! $foo = $this->cache->get('user_'.USER_COOKIE_ID)) {
                // Save into the cache for 500 minutes
                $this->cache->save('user_'.USER_COOKIE_ID, array('result' => $result, 'save' => $save), 30000);
            } else {
                if($this->cache->cache_info()['user_'.USER_COOKIE_ID]['size'] != 0){
                    $this->cache->delete('user_'.USER_COOKIE_ID);
                    $this->cache->save('user_'.USER_COOKIE_ID, array('result' => $result, 'save' => $save), 30000);
                }
            }*/
            echo json_encode(array('status' => 'success', 'value' => $result));
            ...
            if(!is_numeric($task_id)) {
                ...
                if(empty($checkPermEdit)){
                    ...
                    if(isset($data['help_multitask'][0]->multitask_id)) {
                        ...
                        $this->view_library->multitask('help', $data);
                    } else {
                        show_404();
                    }
                }
