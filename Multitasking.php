...
trait WorkInMultitask{
    static function setNameUrl(){
        ...
        } elseif($_SERVER['SERVER_NAME'] == SERVER_NAME_CONST){
    ...
    public function closeTaskUser(){
        ...
        if ($result == 'true') {
            ...
            if($sendArray['status'] == 1){
                $telegram_text = "Задача была обратно вам возвращена. Подробнее: ".WorkInMultitask::setNameUrl()."multitasking/review/" . $task_id;
            } else if($sendArray['status'] == 2){
                $telegram_text = "Задача была отправлена на закрытие. Подробнее: ".WorkInMultitask::setNameUrl()."multitasking/review/" . $task_id;
            }
            if (!empty($respons_task_noti)) {
                foreach ($respons_task_noti as $noti) {
